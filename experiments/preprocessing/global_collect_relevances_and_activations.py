...
def run_collect_relevances_and_activations(config, force_recompute, dataset=None):
    ...
    artifact_type = config.get('artifact_type', None)
    p_artifact = config.get('p_artifact', None)
    p_artifact_na = config.get('p_artifact_na', None)
    p_artifact_landbirds = config.get('p_artifact_landbirds', None)
    p_artifact_waterbirds = config.get('p_artifact_waterbirds', None)
    artifact_extension = f"_{artifact_type}" if artifact_type else ""
    artifact_extension += f"-{config['p_artifact']}" if p_artifact else ""
    artifact_extension += f"-{config['p_artifact_na']}" if p_artifact_na else ""
    artifact_extension += f"-{config['p_artifact_landbirds']}" if p_artifact_landbirds else ""
    artifact_extension += f"-{config['p_artifact_waterbirds']}" if p_artifact_waterbirds else ""
    ...
    # Find sample IDs with given class_idx --- Only works with dataset not with Subset
    ...
    EVERY_N_SAMPLES_IMAGENET = 25
    EVERY_N_SAMPLES_ISIC = 1
    if dataset_name == "imagenet_attacked":
        # For Imagenet, we only want to compute the relevances for every 25th sample
        samples = samples[::EVERY_N_SAMPLES_IMAGENET]
        print(f"Reduced number of samples to {len(samples)} for class {class_idx} in dataset {dataset_name}.")
    elif dataset_name == "isic_attacked":
        samples = samples[::EVERY_N_SAMPLES_ISIC]
        print(f"Reduced number of samples to {len(samples)} for class {class_idx} in dataset {dataset_name}.")
    elif "perfect" in dataset_name:
        len_samples = len(samples)
        split_attacked = samples[:len_samples // 2]
        split_non_attacked = samples[len_samples // 2:]

        # now take every 25th sample from both splits
        if dataset_name == "perfect_cav_imagenet_attacked":
            EVERY_N_SAMPLES = EVERY_N_SAMPLES_IMAGENET
        else:
            EVERY_N_SAMPLES = EVERY_N_SAMPLES_ISIC
        split_attacked = split_attacked[::EVERY_N_SAMPLES]
        split_non_attacked = split_non_attacked[::EVERY_N_SAMPLES]
        samples = np.concatenate((split_attacked, split_non_attacked))

        print(f"Reduced number of samples to {len(samples)} for class {class_idx} in dataset {dataset_name} by taking every {EVERY_N_SAMPLES} of each attakced / clean subset.")
