...
def get_parser():
    ...
    #                     # default="9742,9970,14040,15229,1819", type=str) # ISIC attacked microscope
    #                     # default="12903,3556,17373,15348,20817,10377", type=str) # Chexpert PM (train)
    #                     # default="21,27,54,64,68,71,85,87,100,145,182,188,214,221,230,236", type=str) # Chexpert PM (train)
    #                     default="419,415,427,496,437,441,417,375,475,402,486,501,397,406,468,443", type=str) # ISIC2024 band-aid (train)
    # parser.add_argument("--sample_ids", default="7040", type=str) # hyperkvasir - insertion tube (train)
    ...
    parser.add_argument("--config_file", 
                        # default="config_files/revealing/hyper_kvasir/local/resnet50d_identity_2.yaml")
                        # default="config_files/revealing/hyper_kvasir_attacked/local/resnet50d_identity_2.yaml")
                        # default="config_files/revealing/isic_attacked_microscope/local/resnet50d_last_conv.yaml")
                        # default="config_files/revealing/chexpert/local/vgg16_binaryTarget-Cardiomegaly_pm_features.29.yaml")
                        # default="config_files/revealing/chexpert/local/resnet101d_binaryTarget-Cardiomegaly_pm_last_conv.yaml")
                        default="config_files/revealing/isic/local/resnet50d_identity_2.yaml")
                        # default="config_files/revealing/isic/local/efficientnet_b0_last_conv.yaml")
    parser.add_argument('--savedir', default='plot_files/_extra_crp_plots/')
    return parser
...
def main():
    ...
    run_crp(config, None, args.n_concepts, args.n_refimgs, args.max_mode, args.split, args.savedir)
...
def run_crp(config, sample_ids, n_concepts, n_refimgs, max_mode, split, savedir):
    ...
    sample_ids = dataset.sample_ids_by_artifact["ruler_spray_7"]
    ...
    for sample_id in tqdm.tqdm(sample_ids):
        ...
        valid_concepts = [762]
        spurious_concepts = [420,428,511]
        contains_valid_concept = any([c in topk_ind for c in valid_concepts])
        contains_spurious_concept = any([c in topk_ind for c in spurious_concepts])

        if not (contains_valid_concept and contains_spurious_concept):
            continue
        else:
            print("found sample")
        conditions = [{"y": target, 
    ...
    for r, row_axs in enumerate(axs):
        ...
        for c, ax in enumerate(row_axs):
            ...
            if c == 1:
                ...
                ax.imshow(imgify(cond_heatmap[r], symmetric=True, cmap="bwr", level=level, padding=True))
