...
def get_parser():
    ...
    parser.add_argument('--config_file',
                        # default="config_files/revealing/imagenet/local/resnet18_last_conv.yaml")
                        default="config_files/revealing/isic/local/vgg16_features.24.yaml")
                        # default="config_files/revealing/isic/local/vgg16_features.28.yaml")
                        # default="config_files/revealing/chexpert/local/vgg16_binaryTarget-Cardiomegaly_pm_features.24.yaml")
                        # default="config_files/revealing/hyper_kvasir/local/vgg16_features.28.yaml")
                        # default="config_files/revealing/hyper_kvasir/local/vgg16_features.28.yaml")
...
def main():
    ...
    plot_unsupervised_concept_embedding(config, args.max_mode, args.aggr, args.direction_ids, 
...
def plot_unsupervised_concept_embedding(config, max_mode, aggr, direction_ids, batch_size, split, class_id, savedir):
    ...
    fname_similarities = f"{config['dir_precomputed_data']}/direction_clustering/{config['dataset_name']}_{split}_{config['model_name']}/similarities/{config['layer_name']}_{class_id}.pth"
    ...
    if not os.path.isfile(fname_similarities):
        ...
        for x, y in tqdm.tqdm(dl):
            ...
            acts = attr.activations[config['layer_name']].detach().cpu().flatten(start_dim=2)
            rels = attr.relevances[config['layer_name']].detach().cpu().flatten(start_dim=2)
            activations_avg.append(acts.mean(2).clamp(min=0))
            activations_max.append(acts.max(2).values.clamp(min=0))
            relevances_avg.append(rels.mean(2).clamp(min=0))
            relevances_max.append(rels.max(2).values.clamp(min=0))
    ...
    sim = torch.load(fname_similarities)[sim_key]
    D = (1-sim).clamp(min=0)
    ...
    for algorithm in ["umap", "tsne"]:
        ...
        outlier_concepts = [int(nid) for nid in neuron_ids.split(",")] if neuron_ids is not None else outlier_concepts
        ...
        ref_imgs = fv.get_max_reference(outlier_concepts[:MAX_NUM_CONCEPTS_SHOW], layer_name, "relevance", (0, 5), composite=composite, rf=True,
                                    plot_fn=vis_opaque_img_border)
    

        savename = f"{savedir}/{config['dataset_name']}/{config['model_name']}/{config['layer_name']}_{algorithm}_{aggr}_{max_mode}"
...
def plot_2d(data, label, ax, axis_labels={"x": "UMAP 1", "y": "UMAP 2"}):
    ...
    # sns.scatterplot(
    #     data=data.sort_values("label"), x="x", y="y", 
    #     hue = 'label', 
