...
def get_parser():
    ...
    parser.add_argument("--artifact", type=str, default="pm")
    ...
    parser.add_argument('--savedir', default='results/cav_ranking', type=str,)
...
def main():
    ...
    run_cav_artifact_ranking(config, args.artifact, args.fraction, args.batch_size, args.savedir, args.before_correction)
...
def run_cav_artifact_ranking(config, artifact, fraction, batch_size, savedir, before_correction=False):
    ...
    artifact = config.get("artifact_type", artifact)
    ...
    for layer_name, key in ((ln, k) for ln in layer_names for k in keys):
        ...
        model = get_fn_model_loader(config["model_name"])(n_class=len(dataset.classes), ckpt_path=ckpt_path, device=device).to(device)
        model = model.eval()
        ...
        dataset_for_cav = dataset
        config_cp = copy.deepcopy(config)
        if key == "cav_scope":
            config_cp["cav_scope"] = [1]


        if key == "perfect_cav":
            config_cp["cav_scope"] = None
            if config["dataset_name"] == "imagenet_attacked" or config["dataset_name"] == "isic_attacked":
                config_cp["dataset_name"] = f"perfect_cav_{config['dataset_name']}"
            elif config["dataset_name"] == "waterbirds":
                config_cp["dataset_name"] = "backgrounds"

            cav_a = load_cav(config_cp, artifact, mode="cavs_max").float().to(device)
            cav_r = load_cav(config_cp, artifact, mode="rel").float().to(device)
        else:
            # config["direction_mode"] = "signal"
            cav_a = get_cav_from_model(model, dataset_for_cav, config_cp, artifact, mode="cavs_max").float().to(device)
            cav_r = get_cav_from_model(model, dataset_for_cav, config_cp, artifact, mode="rel").float().to(device)
        ...
        for split in [
            ...
            fname_cav_scores = f"{config['dir_precomputed_data']}/cav_scores/{config['dataset_name']}_{split}_{config['model_name']}/{config['layer_name']}_{artifact}_{fraction}{suffix}.pth"
            x_proj_cav_all = []
            rels_proj_cav_all = []
            x_latent_all = None
            ...
            if EVAL_NMF:
                ## fit NMF
                fname_nmf_directions = f"{config['dir_precomputed_data']}/unsupervised_cavs/{config['dataset_name']}_train_{config['model_name']}/{config['layer_name']}_{artifact}_{fraction}{suffix}.pth"
            ...
            savename = f"{savedir}/distribution/{config['dataset_name']}/{config['model_name']}/{key}/{artifact}_{config['layer_name']}_{split}{suffix}.pdf"
            ...
            # Save as svg
            create_data_annotation_plot(data_pd, dataset_split, idxs_interesting_clean, idxs_interesting_art,
                                    localizations, plot_connections=True, savename=savename[:-4] + ".svg")
            
            create_data_annotation_plot(data_pd, dataset_split, idxs_interesting_clean, idxs_interesting_art,
                                    localizations, plot_connections=False, savename=savename[:-4] + "_no_connections.svg")
