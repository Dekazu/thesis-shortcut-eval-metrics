...
def get_args():
    ...
    parser.add_argument('--config_file',
                        default="config_files/bias_mitigation_real/isic/local/vgg16_band_aid_RRClarc_lamb100000_sgd_lr0.001_features.29.yaml")
...
def compute_accuracy_attacked(config):
    ...
    dataset = load_dataset(config, hm=True, normalize_data=False)
    ...
    for split in ['val', 'test']:
        ...
        for k, samples in enumerate(sample_sets):
            ...
            metrics = compute_metrics(y_pred, y_target, None,
                                      prefix=f"{split}_auto-attacked_{config['attack_artifact']}_",
