...
def get_args(fixed_arguments: List[str] = []):
    ...
    parser.add_argument('--config_file', default="config_files/fixing/local/vgg16_RRR_ExpMax_sgd_lr0.0002_lamb200.yaml")
...
def main():
    args = get_args(fixed_arguments=["dataset_name", "ckpt_path"])
    ...
    gaussian = torchvision.transforms.GaussianBlur(kernel_size=301, sigma=10.0)
    ...
    for split in ['train', 'val']:
        ...
        for k, samples in enumerate(sample_sets_split):
            ...
            for i in tqdm(range(n_batches)):
                ...
                masks = torch.stack([dataset.get_mask(ind) for ind in samples_batch], 0).to(device)
                gaussian_mask = (1 - gaussian(1 - masks))
                multiplicators = torch.tensor([1 / gaussian_mask[i][masks[i] == 1].min() for i in range(len(data))])
                masks = (gaussian_mask * multiplicators[:, None, None, None].to(gaussian_mask)).clamp(max=1)
                data_ = data * masks + (1 - 1 * masks) * data.flatten(start_dim=2).median(2)[0][..., None, None]
