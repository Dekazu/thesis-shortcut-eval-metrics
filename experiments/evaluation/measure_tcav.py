...
def get_args():
    ...
    parser.add_argument('--config_file',
                        default="config_files/evaluate_metrics_clean/isic_attacked/local/isic_attacked_resnet50d_p_artifact0.3_p_artifact_na0.05_last_conv_baseline.yaml")
...
def measure_quality_cav(config, before_correction):
    ...
    artifact_name = config["artifact_type"]
    ...
    keys = ["standard", "perfect_cav", "only_attacked", "only_attacked_perfect_cav"]
    if config["dataset_name"] == "isic_attacked" or config["dataset_name"] == "waterbirds":
        keys.append("cav_scope")

    for layer, key in ((ln, k) for ln in layer_names for k in keys):
        config["layer_name"] = layer

        # Get CAV
        config_cav = config.copy()
        if "perfect_cav" in key:
            config_cav["cav_scope"] = None
            if config["dataset_name"] == "imagenet_attacked" or config["dataset_name"] == "isic_attacked":
                config_cav["dataset_name"] = f"perfect_cav_{config['dataset_name']}"
            elif config["dataset_name"] == "waterbirds":
                config_cav["dataset_name"] = "backgrounds"

            cav = load_cav(config_cav, config['artifact_type'], mode="cavs_max")
        else:
            if "cav_scope" in key:
                config_cav["cav_scope"] = [1]
            cav = get_cav_from_model(model, dataset, config_cav, config['artifact_type'])

        suffix = f"{layer}_{key}"
        ...
        for split in [
            ...
            if "only_attacked" in key:
                if config["dataset_name"] == "waterbirds":
                    split_set = [i for i in split_set if dataset.get_target(i) == dataset.get_class_id_by_name("waterbird")]
                else: 
                    split_set = [i for i in split_set if dataset.get_target(i) == dataset.get_class_id_by_name(dataset.attacked_classes[0])]
            ...
            # controlled setting
            if config["dataset_name"] == "waterbirds":
                attacked_class = dataset.get_class_id_by_name("waterbird")
            else:
            ...
            if "perfect_cav" in key:
                # Print TCAV_pos and TCAV_neg
                logging.info(f"TCAV_pos: {TCAV_pos}, TCAV_neg: {TCAV_neg}")

                logging.info(f"TCAV_pos_mean: {TCAV_pos_mean}, TCAV_neg_mean: {TCAV_neg_mean}")

            results[f"{split}_mean_tcav_quotient_{artifact_name}_{suffix}"] = tcav_metrics['mean_tcav_quotient']
            results[f"{split}_mean_quotient_{artifact_name}_sderr_{suffix}"] = tcav_metrics['mean_quotient_sderr']

            results[f"{split}_tcav_quotient_{artifact_name}_{suffix}"] = tcav_metrics['tcav_quotient']
            results[f"{split}_quotient_{artifact_name}_sderr_{suffix}"] = tcav_metrics['quotient_sderr']

            results[f"{split}_mean_tcav_sensitivity_{artifact_name}_{suffix}"] = tcav_metrics['mean_tcav_sensitivity']
            results[f"{split}_mean_tcav_sensitivity_{artifact_name}_sem_{suffix}"] = tcav_metrics['mean_tcav_sensitivity_sem']
