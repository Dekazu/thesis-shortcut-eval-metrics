...
def plot_corrected_model(config, sample_ids, normalized, plot_to_wandb, path):
    ...
    if sample_ids is None:
        ...
        # TODO: maybe define for waterbirds a few artifact samples (water background) and a few clean samples (land background) beacuse world-sampled artefact
        sample_ids = dataset.sample_ids_by_artifact[config['artifact_type']][:5]
    ...
    for i, sample_id in enumerate(sample_ids):
        ...
        axs[0, i].set_xticks([])
        axs[0, i].set_yticks([])
        axs[0, i].set_title(f"Sample {sample_id} -- Target {dataset.map_target_label(target[i].item())}",fontdict={'fontsize': 9})
        # axs[0, i].axis("off")
    ...
    plt.savefig(f"{path}/{config['wandb_id']}.pdf", bbox_inches="tight", dpi=400)
    plt.savefig(f"{path}/{config['wandb_id']}.svg", bbox_inches="tight", dpi=400)
    ...
    plt.savefig(f"{path}/{config['wandb_id']}_no_la‚bels.jpeg", bbox_inches="tight", dpi=200)
    plt.savefig(f"{path}/{config['wandb_id']}_no_labels.pdf", bbox_inches="tight", dpi=400)
    plt.savefig(f"{path}/{config['wandb_id']}_no_labels.svg", bbox_inches="tight", dpi=400)
    plt.show()
