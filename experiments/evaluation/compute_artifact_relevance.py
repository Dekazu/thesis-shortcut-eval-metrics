...
def get_args():
    ...
    parser.add_argument('--config_file',
                        default="config_files/evaluate_metrics_clean/imagenet_attacked_clean/local/imagenet_attacked_clean_ch_time_vgg16_p_artifact0.5_features.29_baseline.yaml"
                        )
...
def main():
    ...
    compute_artifact_relevance(config, args.before_correction)


def compute_artifact_relevance(config, before_correction):
    """
    ...
    for split in ['val', 'test']: # ['train', 'val', 'test']
        ...
        if attacked_classes is not None:
            # Only consider attacked classes
            sample_sets_split = [[x for x in y if dataset.classes[dataset[x][1]] in attacked_classes] for y in sample_sets_split ]
