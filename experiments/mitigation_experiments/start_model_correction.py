...
from utils.cav_utils import load_cav
...
def get_parser():
    ...
    parser.add_argument('--use_bg_cav', action='store_true',
                        help='Use background CAV for ClArC correction.')
    return parser
...
def main():
    ...
    start_model_correction(config, args.num_gpu, use_bg_cav=args.use_bg_cav)


def start_model_correction(config, num_gpu, use_bg_cav=False):
    ...
    if "clarc" in method.lower():
        ...
        kwargs_correction['artifact_sample_ids'] = dataset.sample_ids_by_artifact[config['artifact_type']]
    ...
    # Define Optimizer and Loss function
    model_corrected.set_optimizer(optimizer_name, model_corrected.parameters(), lr, ckpt_path, device=device)

    weights = None if dataset_name == "imagenet" else dataset.weights
    model_corrected.set_loss(loss_name, weights=weights)

    if use_bg_cav:
        config_bg_cav = config.copy()
        config_bg_cav['dataset_name'] = "backgrounds"
        bg_cav = load_cav(config_bg_cav, config['artifact_type'], mode="cavs_max")
        model_corrected.cav = bg_cav.clone().detach().cpu() 
