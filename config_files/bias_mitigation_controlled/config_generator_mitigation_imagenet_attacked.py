...
config_dir = "config_files/bias_mitigation_controlled/imagenet_attacked"
...
with open("config_files/chosen_model_params.json", "r") as stream:
    chosen_model_params_frog = json.load(stream)["imagenet_attacked_frog"]
with open("config_files/chosen_model_params.json", "r") as stream:
    chosen_model_params_dog = json.load(stream)["imagenet_attacked_dog"]
...
def store_local(config, config_name, artifact_type):
    ...
    if artifact_type == "ch_time":
        optim_name, lr, p_backdoor = chosen_model_params_frog[model_name][str(p_artifact)]
    elif artifact_type == "bone":
        optim_name, lr, p_backdoor = chosen_model_params_dog[model_name][str(p_artifact)]
    p_artifact = config['p_artifact']
    
    config['ckpt_path'] = f"{path_local_checkpoints}/imagenet_attacked_{artifact_type}_{model_name}_{optim_name}_lr{lr}_p_artifact{p_artifact}_p_backdoor{p_backdoor}.pth"
...
def store_cluster(config, config_name, artifact_type):
    ...
    config['ckpt_path'] = f"checkpoints/imagenet_attacked_{artifact_type}_{model_name}_{optim_name}_lr{lr}_p_artifact{p_artifact}_p_backdoor{p_backdoor}.pth"
...
for model_name, layer_names in [
    ...
    p_artifact_rates = [ 0.3, #0.2, 0.4, 0.5
                        ]
    epochs = [5, 10, 20] #[5, 10, 30]
    for epoch in epochs:
        for p_artifact in p_artifact_rates:
            for layer_name in layer_names:
                ...
                config_dog['criterion'] = "all_logits_random"
                config_dog['cav_scope'] = [0]
                
                config_dog['p_artifact'] = p_artifact

                optim_name, lr, backdoor = chosen_model_params_dog[model_name][str(p_artifact)]
                ...
                for lamb in lambs:
                    config_dog["lamb"] = lamb
                    config_name = f"imagenet_attacked_{artifact_type_dog}_{model_name}_{method}_lamb{lamb:.0f}_{optim_name}_lr{lr}_ep{epoch}_p_artifact{p_artifact}_{layer_name}"
                ...
                config_frog["criterion"] = "all_logits_random"
                config_frog['cav_scope'] = None

                
                config_frog['p_artifact'] = p_artifact

                optim_name, lr, backdoor = chosen_model_params_frog[model_name][str(p_artifact)]
                config_frog['optimizer'] = optim_name
                config_frog['lr'] = lr

                artifact_type_frog = 'ch_time'
                config_frog['artifact_type'] = artifact_type_frog
                #config_frog['lsb_trigger'] = lsb_trigger
                #config_frog['lsb_factor'] = 4
                ...
                for lamb in lambs:
                    config_frog["lamb"] = lamb
                    config_name = f"imagenet_attacked_{artifact_type_frog}_{model_name}_{method}_lamb{lamb:.0f}_{optim_name}_lr{lr}_ep{epoch}_p_artifact{p_artifact}_{layer_name}"
