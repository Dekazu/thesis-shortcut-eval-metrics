...
class ISICDataset(BaseDataset):
    ...
    def __init__(self,
        ...
        if self.binary_target:
            ...
            # Add Benign and Malignant classes
            Benign = ['NV', 'BKL', 'DF', 'VASC']
            Malignant = ['MEL', 'BCC', 'SCC', 'AK']

            benign = [self.metadata[b] == 1.0 for b in Benign]
            malignant = [self.metadata[m] == 1.0 for m in Malignant]
            self.metadata['Benign'] = np.where(benign[0] | benign[1] | benign[2] | benign[3], 1.0, 0.0)
            self.metadata['Malignant'] = np.where(malignant[0] | malignant[1] | malignant[2] | malignant[3], 1.0, 0.0)
            #self.metadata.drop(columns=Benign + Malignant, inplace=True)

            dist = np.array([float(x) for x in self.metadata.agg(sum).values[-len(self.classes):]])
        ...
        self.sample_ids_by_artifact = self.get_sample_ids_by_artifact() # TODO: ??

        self.all_artifact_sample_ids = [sample_id for _, sample_ids in self.sample_ids_by_artifact.items() for sample_id
                                        in sample_ids] # TODO: ??
        self.clean_sample_ids = list(set(np.arange(len(self))) - set(self.all_artifact_sample_ids)) # TODO: ??
    ...
    def get_target(self, i):
        ...
        if self.binary_target:
            # 1 = Malignant, 0 = Benign
            target = (targets['Malignant'] == 1).astype(int)
        else: 
            target = self.classes.index(targets[targets == 1.0].index[0])
