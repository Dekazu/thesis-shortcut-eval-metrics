...
class ISICAttackedHmDataset(ISICAttackedDataset):
    def __init__(self, 
        ...
        # Add Segmentation Path to metadata if exist else nan
        if self.gt_segmentation_path:
            for i, row in self.metadata.iterrows():
                path =  self.gt_segmentation_path / Path(row['image'].replace('_downsampled', '') + '_segmentation' + '.png')
                if path.exists():
                    self.metadata.at[i, "segmentation_path"] = path
                else:
                    self.metadata.at[i, "segmentation_path"] = 'nan'
        else:
            print("No segmentation path provided")
            self.metadata["segmentation_path"] = 'nan'
    ...
    def get_segmentation_mask(self, i):
        row = self.metadata.loc[i]
        if row.segmentation_path == "nan":
            return np.nan
        img = Image.open(row.segmentation_path)
        img = img.convert('L')

        tensor_transform = T.ToTensor()
        mask = tensor_transform(img).float()

        mask = T.Resize((self.image_size, self.image_size), interpolation=T.functional.InterpolationMode.BICUBIC)(mask)
